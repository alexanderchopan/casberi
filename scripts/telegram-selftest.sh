...
swiftc -Onone -o "$TMP/run" "$CHANNEL" "$TMP/support.swift" "$TMP/main.swift" \
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "$TMP/TelegramChannel.swift" "$TMP/support.swift" "$TMP/main.swift" 2>/dev/null; then
