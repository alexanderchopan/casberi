...
# every individual page. Four separate things made that one number wrong, and
# this holds all four: Privy's app wallets are OUT of the total (an app's money
# is stated in the app's own room, and it was the one stored read standing
# beside live ones); the dust floor is the answering arm's, since $1.99 predates
# Zerion's trash filter and on that arm drops only the person's real small
# positions; a chain filter is built from the caller's networks and a failed
# filtered read retries unfiltered; a rejected Alchemy body is isolated one
# network at a time and the refusal is LEARNED rather than hand-listed. And a
# pass that reached nothing stands on the last known reading, with a date on it.
step "Wallet-total audit"
python3 "$ROOT/scripts/wallet-total-audit.py" --self-test >/dev/null \
  || fail "the wallet-total audit's own self-test failed — the check is broken, not the code"
python3 "$ROOT/scripts/wallet-total-audit.py" \
...
# A landed row carries its door (prd §909, §912). `Thing.init` defaults
...
# constructions catches it; the root-page case is §912's per-bridge list.
...
# Duolingo's practice-day door (prd §776) — the least-measured live seat in the
# tree, so the harness is the only thing holding its parse at all: a JWT payload
# read as plain base64 is nil for every token carrying a `-`, a day stamped at
# its UTC midnight files under yesterday for every reader east of Greenwich, a
...
# after Apple's macOS review named the surface that advertised it. Four things
# a build or a screen sweep cannot see: EVERY prompt must carry the answer-only
# rail, and carry it above the question; there must be exactly one prompt
# builder and one verb, or there is a path to the wire that walks around the
# rail; corpus text must never ride along; and nothing anywhere may promote
# this seat or offer an action in its copy. Every failure renders as a
# perfectly ordinary screen. `-bankrProbe` is the live proof; this is the
