...
# `scripts/support/harness-opt-probe.sh` before the swap (2026-09-05, 4.6x faster).
# Re-probe before trusting it again after adding mutations.
if ! swiftc -Onone -o "$TMP/cr-selftest" "$ROOM" "$LEDE" "$TMP/main.swift" 2>"$TMP/build.log"; then
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "$target" "$LEDE" "$TMP/main.swift" 2>/dev/null; then
