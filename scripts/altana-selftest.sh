...
# Since prd §919 the block applies `DSRoomHeadWell`, a modifier whose body
# draws the well — either spelling passes, and a block that draws neither
# fails.
{ perl -0pe 's{//[^\n]*}{}g' "$TEMPLATE" | grep -A5 'func dsRoomHeadBlock() -> some View' | grep -q 'dsWell(' ||
  { perl -0pe 's{//[^\n]*}{}g' "$TEMPLATE" | grep -A5 'func dsRoomHeadBlock() -> some View' | grep -q 'DSRoomHeadWell()' &&
    perl -0pe 's{//[^\n]*}{}g' "$TEMPLATE" | grep -A12 'struct DSRoomHeadWell: ViewModifier' | grep -q 'dsWell('; }; } \
  || { echo "✗ the room head block no longer draws the lead's well — prd §766 (through DSRoomHeadWell since §919)"; exit 1; }
...
swiftc -Onone -o "$TMP/selftest" "$MODEL" "$ROOM" "$SHEET" "$DISC" "$DRIVER"
"$TMP/selftest"
