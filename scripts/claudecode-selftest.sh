...
if ! swiftc -Onone -o "$TMP/run" "$TMP/extracted.swift" "$TMP/main.swift" 2>"$TMP/build.log"; then
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "$WORK/extracted.swift" "$TMP/main.swift" 2>/dev/null; then
