...
# `scripts/support/harness-opt-probe.sh` before the swap (2026-09-05, 9.5x faster).
# Re-probe before trusting it again after adding mutations.
if ! swiftc -Onone -o "$TMP/run" "$ROOM" "$SHARED_RUNWAY" "$TMP/main.swift" 2>"$TMP/build.log"; then
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "$WORK/DodoPaymentsRoom.swift" "$SHARED_RUNWAY" "$TMP/main.swift" 2>/dev/null; then
