...
if ! swiftc -Onone -o "$TMP/run" "$TMP/WalletActionMark.swift" "$TMP/main.swift" 2> "$TMP/build.log"; then
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "$WORK/WalletActionMark.swift" "$TMP/main.swift" 2>/dev/null; then
