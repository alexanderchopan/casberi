...
# `scripts/support/harness-opt-probe.sh` before the swap (2026-09-05, 3.4x faster).
# Re-probe before trusting it again after adding mutations.
if ! swiftc -Onone -o "$TMP/run" "${SOURCES[@]}" "$TMP/main.swift" 2>"$TMP/build.log"; then
...
PY
  ...
  if ! swiftc -Onone -o "$TMP/mut" "${copies[@]}" "$TMP/main.swift" 2>/dev/null; then
