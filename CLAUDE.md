...
- `-byokKey <key|clear>` — store (or clear) the BYO Anthropic key headlessly (Keychain via TokenVault); `-byokProbe "<query>"` — run the keyed answer path (retrieve → device→api.anthropic.com → doc) and NSLog the result — with a bogus key it verifies the honest-failure path (HTTP 401 → nil) without spending anything. Settings detail: `-accountDetail key`.
