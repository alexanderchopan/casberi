...
- **Safe, GitHub and Stripe carry kind tiles on the one template (prd §815): the cover (always the full box, `holdsLead: true`), `DSScopeTiles` `contentGap` under it, the list `leadGap` under them. All is first and where the room opens; fewer than two kinds draws no tiles; a glyph carries ONE meaning across every tile and dock seat (`Model/RoomKindTiles.swift`, `room-kind-tiles-selftest.sh`).** Safe's and Stripe's head cards are deleted; the Safe module warning rides the cover's note → prd §815
