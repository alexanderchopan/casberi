...
- **A room-level act rides the ALL card, never the per-account ones (prd §744).** The Wallet's `Follow address` is a DOOR to `WalletWatchField` (§466 stands — one watch field; the four bridge setup screens still say `Watch a wallet` for the same destination), and Frames' Send/Top up sit there because the device holds one key → prd §744
