...
- **The wallet family has NO BAR (prd §744, supersedes §547).** The account is a card you page (`DSAccountDeck`/`DSAccountSlot`), carrying the room's crown and its acts; the readings are door rows (`DSScopeRows`, on `DSPushRow`) that state what they hold BEFORE the tap; a pushed scope wears a SCROLL header (`DSScopeHeader`), never a swipe — a swipe is the room's (§648/§663). One drop-in for all five rooms: `DSRoomScopeChrome`. Wallet and Frames are converted; Vibenet, Hegotá and Ethrex Privacy still mount `DSRoomRailSlab` → prd §744
- **A room-level act rides the ALL card, never the per-account ones (prd §744).** The Wallet's `Watch a wallet` is a DOOR to `WalletWatchField` (§466 stands — one watch field), and Frames' Send/Top up sit there because the device holds one key → prd §744
