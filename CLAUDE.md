...
- **Safe's keyless quota is ONE shared pool, it is empty, and a REAL KEY IS SERVED AS ANONYMOUS (prd §789 · §789a, measured twice).** A refused read is never "nothing": every tx-service GET goes through `SafeServiceGate` (`ok`/`missing`/`throttled`/`unreachable`, a 429 shuts the gate until `x-ratelimit-reset` and persists it), and a surface may say "nothing" only about a pass that answered (`mark()` then `health(since:)`, `safe-gate-selftest.sh`). The key measured 429 with the keyless tier's own `x-ratelimit-limit: 5000` on both `v1` and `v2`, so **`shippedKey` stays empty and `NetworkReach` keeps saying keyless** — do not fill the constant without a keyed 200, and prove one with `-safeKey "$(scripts/dev-keys.sh get safe)"` first. The lever with evidence is FEWER READS: detection and config come off the chain, the owner reverse-lookup caches its empties (its `AddedOwner` event is unindexed before Safe 1.4.1, so it cannot leave the service), and the QUEUE read never can — a proposed transaction is off-chain → docs/hooks/wallet.md · prd §789 · §789a
