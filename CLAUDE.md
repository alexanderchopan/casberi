...
- **A seat the store holds can always be spoken about and always be left (prd §784).** A refusal clears a session seat's own credential (§711), so reading the credential first made the page say "Not connected" over a catalogue row saying "Needs reconnecting · Fix" — and the page's exits are gated on the state, so the warning had no Pause and no Disconnect behind it. The derivation is `AccountPageShape.state(_:)` (Foundation-only, `account-page-selftest.sh` compiles it): the store's word first, the credential only deciding reading from not connected; the exits draw for any registered seat. A seat's attention line is recognised by IDENTITY against `BridgeHealth.attentionLine` (it ships in five languages), and resuming a shut-out seat returns it to `.attention` rather than claiming a sync → prd §784
