...
- `-byokKey <key|provider:key|clear>` — store a BYO AI key headlessly (Keychain, one slot per provider). A bare key's provider is detected from its prefix (`AIProvider.detect`); prefix-less providers take the `provider:` form (e.g. `venice:<key>`); `clear` removes every provider's key. `-byokProbe "<query>"` or `"provider:<query>"` — run the keyed answer path (retrieve → device→provider API direct → doc) against the named (or first connected) provider and NSLog the result — with a bogus key it verifies the honest-failure path (non-200 → nil) without spending anything. Four providers (Claude/GPT/Gemini/Venice), each also connectable from its app page in Apps (`Screens/AIKeySetupScreen.swift`); wire details in `Model/AIProvider.swift` + `Model/AIAnswer.swift`. Settings overview: `-accountDetail key`.
