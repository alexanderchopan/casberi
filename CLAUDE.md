...
- **An empty scope draws its own figure as a still skeleton with its short state centred on it, and its list as skeleton rows (prd §769).** `DSEmptyState(.room(figure))` over `DSSkeletonFigure`, one figure per `ScopeTileGlyph` name; `DSSkeletonRows` wherever a list arm would draw nothing, gated on that arm's own condition. No shimmer, no veil, no sub line — `emptyBody` is the VoiceOver value → prd §769
