...
- **A ripple marks something entering the water, never a hand (prd §804): a landed wallet row rings once from its lead (`WalletArrival.pulse(for:)` → `DSFeedRow.arrival`), a live chart endpoint rings every 2.4s (`dsLiveRipple`, behind `freshWindow`). `Design/DSRipple.swift`: a radial fill never a stroke, `DS.rippleInk`, Core Animation, nothing under Reduce Motion. Not under a press, the dock, the refresh, an empty state, a ground or a sheet** → docs/hooks/design.md · prd §804
