...
- **Day-divider audit (scripts/day-divider-audit.py, 2026-09-15) — the day header wears `DS.brandInk` (the mark's pink); a group named by something other than time passes `dated: false` and keeps the primary ramp** → docs/verify.md · prd §735
...
- **The day divider is the one line of type in the brand pink (`DS.brandInk`), and the cover card is not.** The hue is the app's own voice, so it never lands on a thing's title, a figure, or a `deckFill` ground (~1.8:1) → docs/hooks/design.md · prd §735
