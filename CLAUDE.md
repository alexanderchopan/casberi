...
- **A room-level act rides the ALL card, never the per-account ones (prd §744).** EVERY door to `WalletWatchField` says `Follow address` — the wallet room's row and the Peer/Privacy Pools/Railgun/Safe setup screens — and the field's own armed verb is `Follow` (§466 stands: one watch field), and Frames' Send/Top up sit there because the device holds one key → prd §744
