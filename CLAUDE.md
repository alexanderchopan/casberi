...
- **One explaining sentence per screen, at most, through `DSFootnote` (prd §747).** A line stays only if it says what the controls cannot; honesty, money, signing, privacy and fix-it lines are kept on purpose. `footnote-audit.py` counts per file, with reasoned allowances → prd §747
