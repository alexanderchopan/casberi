...
- **A digest is written to FIT the lock screen — a title of the place and its most telling number (`Farcaster: 5 replies +2`, `Wallet: +$1,240`), two body lines of short facts (who acted, a count), an app named once — never cut with an ellipsis. The thumbnail stays faces and app tiles (a drawn figure was built and reversed). It goes out at the learned evening reading hour, and its long press is the NotificationContent extension drawing `NotifyCard`, which reads no store and no network. **The extension carries NO entitlement** — every picture is a notification attachment (`head`, `face-N`), because an app-group link on a new identifier is a portal-only step that blocked the ship** → prd §809 · §809a
...
- **Safe's keyless quota is ONE shared pool, it is empty, and a REAL KEY IS SERVED AS ANONYMOUS (prd §789 · §789a, measured twice).** A refused read is never "nothing": every tx-service GET goes through `SafeServiceGate` (`ok`/`missing`/`throttled`/`unreachable`, a 429 shuts the gate until `x-ratelimit-reset` and persists it), and a surface may say "nothing" only about a pass that answered (`mark()` then `health(since:)`, `safe-gate-selftest.sh`). A real key, no key and `Bearer not-a-real-key` all answered 429 with the keyless tier's own `x-ratelimit-limit: 5000`, so **the header is never read while the pool is empty** and **`shippedKey` stays empty with `NetworkReach` still saying keyless** — the key is unproven, not dead (only a 401 would say dead, and the edge sends none); retry it when the pool reopens and ask whether `x-ratelimit-limit` is still 5000, and prove any key with `-safeKey "$(scripts/dev-keys.sh get safe)"` before filling the constant. **The lever was not fewer reads — it was a different host (§789b, 2026-09-17).** Safe's own Client Gateway (`safe-client.safe.global`, what `app.safe.global` reads) answers all six chains keyless, publishes no `x-ratelimit` header, declares no auth scheme in its own OpenAPI, and serves ALL FOUR reads — including the owner lookup, which has no on-chain equivalent (`AddedOwner` is unindexed before Safe 1.4.1). Cutting our own calls could never have helped: the 5,000 is shared with every keyless caller on earth, so the pool is drained by strangers. Its only cap is a per-IP BURST (~60 per short window, refilling in ~20s), which is why `SafeServiceLedger.defaultClosed` is 60s and not 15min, and why the queue's per-transaction detail reads are sequential. `SafeGatewayShape` is the whole translation and the four silent traps are named there (a wrapped address — except `gasToken`; a `null` guard; millisecond times; furniture rows in the queued list); `safe-gateway-selftest.sh` drives it off measured fixtures. The one WRITE stays on `api.safe.global`, which is where a signature has to land → docs/hooks/wallet.md · prd §789 · §789a · §789b
...
- **Every row in every room is one anatomy (prd §764): title `body17` regular, line `subhead12`, money `price17` trailing the title, time `subhead12` on its baseline, section name `heading24`.** Weight carries a fact (the next event, a state word), never a row's title; `label12` medium is for stamps and tags → prd §764
...
- **The dock's face opens ACCOUNTS, and Settings is that screen's third section — search, then `Manage | Connect | Settings` on one switcher, nothing pushed (prd §796).** Both seats `toggle(.apps)` (§705's toggle stands); `HomeRoute.Node.settings` is deleted and the three direct doors set `route.openSettings` beside `present(.apps)`; `SettingsScreen` is `SettingsRows`, and its rows wear the app row's anatomy (`SettingsTile` lead) → prd §796
