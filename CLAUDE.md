...
- **The shell is not built before the window has a width (prd §774).** The root lays out once at zero width; everything survives that pass except a `List`'s self-sizing cells, which cache it — so the feed drew every row at its MINIMUM intrinsic width until the launch row budget lifted and rebuilt it, about two seconds. `shellBase` gates on `shellWidth > 0`; `launchTimer shellWidth` reports the first geometry the shell was handed → prd §774
