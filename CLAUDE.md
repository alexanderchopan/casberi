...
- **Every empty state draws what would fill it, empty (prd §769, §770).** `DSEmptyState` is `.room(figure)` — the scope's own `DSSkeletonFigure`, one per `ScopeTileGlyph` name — or `.list(rows:)` over `DSSkeletonRows`; the short state is centred on it, `words` is VoiceOver's, and `note` is only for a fix-it or honesty fact. No shimmer, no veil. An empty wallet Holdings or Activity list carries `Follow address`, and NFTs `Choose collections` → prd §769 · §770
