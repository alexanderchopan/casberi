...
- **The dock's face opens ACCOUNTS, and Settings is a gear in the Accounts head row — search, `Manage | Connect`, settings (prd §796).** Both seats `toggle(.apps)` (§705's toggle stands); the gear is the only touch door to Settings and it `push`es, so the seat lands you back on Accounts; the tail tile is a second door to Accounts, left standing on purpose (`dock-selftest.sh`) → prd §796
