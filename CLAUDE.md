...
- `-byokKey <key|clear>` — store (or clear) the BYO AI key headlessly (Keychain via TokenVault); the provider (Claude / GPT / Gemini) is detected from the key's prefix (`AIProvider.detect`) and stored alongside. `-byokProbe "<query>"` — run the keyed answer path (retrieve → device→provider API direct → doc) and NSLog the result — with a bogus key it verifies the honest-failure path (non-200 → nil) without spending anything. Providers/wire details live in `Model/AIProvider.swift` + `Model/AIAnswer.swift`. Settings detail: `-accountDetail key`.
