...
- **World Chain is in the wallet's chain picker and OFF by default — the first row there to land UNMEASURED (the Portfolio `by-address` call 400s the whole holdings read on a chain it refuses)** → docs/hooks/wallet.md · prd §785
- **This phone as a World ID AUTHENTICATOR is specced, not built — World ID 4.0 makes a World ID an account with up to 48 keys, `insertAuthenticator` has no allowlist, and the credential is re-issued to a second key through WIP-103; four measurements gate stage 1** → docs/worldid-authenticator-spec.md · prd §785
- **A World ID SEAT is specced, not built (prd §786) — your own account's keys, its recovery agent and every change to them, off one `eth_getLogs` filtered on your `leafIndex`. A different subject from §785's read, which keeps its no-seat ruling (§515a). Thirteen registration points, traced through `"Safe"`; measure `getPackedAccountData` on a World App address FIRST — if it is not an admin authenticator, the seat needs a different door** → docs/worldid-seat-spec.md · prd §786
