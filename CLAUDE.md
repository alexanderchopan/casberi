...
- **The wallet family's Home is one surface: the head, Actions, Readings (prd §750).** Verbs are `DevnetVerbRow`s, never tiles; the accounts are `ShellChrome.accountRail`, drawn by the shell as the face rail above the dock like the social rooms, and every face rail shows whenever it has more than one to pick → prd §750
