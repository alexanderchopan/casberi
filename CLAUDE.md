...
- **The wallet family has NO BAR (prd §747, supersedes §547).** The account is a card you page (`DSAccountDeck`/`DSAccountSlot`), carrying the room's crown and its acts; the readings are door rows (`DSScopeRows`, on `DSPushRow`) that state what they hold BEFORE the tap; a pushed scope wears a SCROLL header (`DSScopeHeader`), never a swipe — a swipe is the room's (§648/§663). ONE template for all five rooms — Wallet, Hegotá Frames, Hegotá UTXO, Ethrex Privacy, Base Vibenet — and `DSRoomRailSlab` is DELETED → prd §747
