...
- No hairlines — one exception: the Apps page draws a single `fillLine` divider between the CONNECTED strip (management) and Discover (store), per the "Apps page — store anatomy" ruling in docs/prd.md. Nothing else draws a line. Widget/tile radius = `DS.Radius.widget`.
