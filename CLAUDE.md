...
- **Whether an address belongs to a VERIFIED HUMAN, keylessly — World ID's public address book on World Chain, no seat and no account (`-worldIDProbe`). Only `verified` and `lapsed` draw: a zero means that book holds nothing for this address, never "not a person" (§83); a read that did not answer is never written, and one we could not READ is kept as `unreadableSeconds` so it is not re-asked forever. Its host is its OWN (`worldchain.drpc.org`) because a receipt resolves its service BY HOST** → docs/hooks/wallet.md · prd §784 · §784a
...
- **A World ID SEAT is specced, not built (prd §785) — your own account's keys, its recovery agent and every change to them, off one `eth_getLogs` filtered on your `leafIndex`. A different subject from §784's read, which keeps its no-seat ruling (§515a). Thirteen registration points, traced through `"Safe"`; measure `getPackedAccountData` on a World App address FIRST — if it is not an admin authenticator, the seat needs a different door** → docs/worldid-seat-spec.md · prd §785
