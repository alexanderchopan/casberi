...
- `-pinHomeSource <source>` — pins a SOURCE to Home (the "Pin to Home" verb / `HomePinnedSources`, e.g. `RSS`, `Pinterest`, `Bluesky`, `Farcaster`) — headless test of a pin-governed board module. Distinct from `-pinSource` (which pins one thing to the Pinned widget).
