...
## 128. The Wallet screen scopes to one wallet — a switcher, whole-screen (user, 2026-07-18) — DEVICE-VERIFIED 2026-07-18

Answers "how does a wallet feed work with more than one wallet — separate or combined? have we thought about it?" The answer was already "combined stream with per-row wallet tags, plus a per-wallet + `All wallets` decomposition on Home and in the `Across your wallets` sheet" (§72/§125/§126) — but the Wallet screen itself had no way to say "show me just this wallet." Ruling (picked over a feed-only filter): the switcher scopes the WHOLE screen, not just the activity list — a feed-only filter is incoherent (the combined value headline still reads across all wallets while the activity below shows one), and tapping a wallet should mean "show me this wallet," full stop.

`WalletScreen` gains a `selectedWallet: String?` (nil = All) driven by a chip strip at the top — `All` then one chip per watched wallet, each wearing that wallet's `WalletFace` and, when selected, its `WalletFace.tint`, the same identity the treemaps/rows/allocation bar already carry (fill-only selection — the design law draws no lines). Only present with more than one wallet watched (one wallet's own view already IS the whole screen). Every content section reads the scope: the combined `Across your wallets` card is HIDDEN when scoped (a combined total beside one wallet's holdings is the exact incoherence this removes); the holdings treemap stack, the NFT shelves, and the Approvals rows filter to the selected wallet; the Recent activity list is a new scoped subview (`WalletRecentList`) whose `@Query` fetches for exactly the selected wallet — NOT an in-memory filter of the fixed 12-row all-wallets fetch, which would under-show (11 rows another wallet's, 1 this one's). The Watching roster stays full — it's the management surface AND the tab-strip; the switcher chips indicate active scope. The old `walletIdentityLegend` above the treemap stack is deleted — the switcher carries wallet identity now, and each treemap card already names its wallet.

The ENS "gap" flagged in the sketch does NOT exist: `WatchedAddress.address` stores resolved hex (the `watch()` UI resolves ENS/`.sol` before `add()`), and `WalletIngest.resolvedAddresses` returns an already-hex/base58 address UNCHANGED, so `Thing.walletAddress` equals the watched `address` byte-for-byte in every production path — an exact `==` predicate is both correct and base58-safe (never lowercase Solana). `sameAddress` (hex compares case-insensitively per EIP-55, everything else exactly) mirrors `WalletStore.dedupeKey` for the in-memory filters. NOTE: the `-walletAddress` DEBUG hook calls `add()` directly with the RAW string, so `-walletAddress "vitalik.eth"` stores a NAME (bypassing resolution) — a hook artifact where the scoped Recent query (matching `Thing.walletAddress`, the resolved hex) finds nothing; seed with HEX to test the production path. Selection returns to All when the selected wallet is removed or the list drops to one (`.onChange(of: addresses)`).

DEVICE-VERIFIED 2026-07-18 (iPhone 17 Pro sim, two hex wallets — vitalik `0xd8dA…6045` + Binance-14 `0x28C6…1d60`): the chip strip renders `All · 0xd8dA…6045 · 0x28C6…1d60` with resolved ENS avatars; tapping a wallet highlights its chip and scopes the treemap ("$19K across 11 tokens"), the NFT shelf (vitalik.wei, Vitalik RGB #1), Approvals (that wallet only), and Recent (its own transfers, no wallet-label sublines) while the `Across your wallets` combined card ($393.1M) disappears; tapping `All` restores the combined card + both wallets in every section. `Screens/WalletScreen.swift` (`switcherSection`/`switcherChip`, `selectedWallet`, `isSelected`/`sameAddress`/`chipIsOn`, `visibleTotals`/`visibleNFTs`, `WalletRecentList`). Scope is the WalletScreen (the wallet's home) only — the separate `casberi://feed/source/Wallet` feed (§124) is unchanged.
