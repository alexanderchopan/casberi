...
## 116. Home: quiet the bento's chrome, restore one synthesized line (2026-07-18) — BUILT, device-verification pending
...
NOT YET DEVICE-VERIFIED (no sim in the authoring environment): the pin's hold-to-reveal feel, and the on-device line's voice/quality/decline-rate over a real corpus (the Foundation Models call can't run headless here). The line's honesty rail is the NONE escape + grounding; tune the prompt on-device. `Model/HomeInsightStore.swift`, `Model/OnDeviceModel.swift` (homeInsight + `HomeNoticeLayout`), `GenUI/HomeComposition.swift`, `Screens/HomeScreen.swift`.

## 117. Home is a tool, not a pinboard — auto-pin + invert the hierarchy (user, 2026-07-18) — BUILT, device-verification pending
...
NOT YET DEVICE-VERIFIED (no sim here): the whole feel — how full an auto-pinned board is, whether the away card/N-new reads as signal or noise for a feed-heavy corpus, board depth under many tiles. Test with `-awayGap <hours>` to exercise the away paths headlessly. `GenUI/HomeComposition.swift`, `Screens/HomeScreen.swift`, `Model/HomePinnedSources.swift`, `Screens/PinToHomeButton.swift`, `GenUI/GenRenderer.swift` (Insight eyebrow).
