...
Collapse (decided with the user, in two rounds): **the one blue intelligence card COMBINES "Just landed", "While you were away", and "Noticed"** — the cover hero and the two synthesis cards were all conveying "here's what's new / what the app noticed", so they became ONE card of three eyebrow-and-line sections (`Insight` grew to three sections, args 0/1, 2/3, 4/5; each skipped when its line is empty, the card omitted only when all three are). Section 1 "Just landed · <source>" is the newest thing that actually landed (capturedAt ≤ now — a future event isn't "just landed"), tappable to open it (arg 6); section 2 the firehose-excluded away read ("10 new, mostly from Calendar and ChatGPT"); section 3 §116's model connection. The **cover is now just the date header + kind chips** (`GenCover` draws no hero card when its title is empty). The "Coming up" card is retired — dated events/reminders read off their own source tiles ("On your calendar", "Reminders" already show what's soon), the person's pick to keep the calendar as a tile rather than duplicate it up top. So the head is **date/chips header → the one blue card → the board**, which now sits above the fold. `appendAway`/`appendComingUp` deleted (the away read survives as `awayLine`); the `ComingUp` model + `GenComingUp` renderer stay (the `-comingUpProbe` still exercises the model, and the flat renderer is retained rather than re-derived if a dated lane returns).
