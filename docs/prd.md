...
**Peer and Privacy Pools are not separate apps you connect. They ride the watched wallets automatically — watching a wallet IS the consent to read its fills/deposits.** Both seats settle into (Peer) or deposit from (Privacy Pools) the person's OWN wallet, so a wallet in one of them is already a wallet you'd watch; the old opt-in toggle (§113/§162's "connecting is one switch") and the "viewing is not consent" gate are gone. This supersedes that specific corner of §113/§162 — the reads, hosts, and honesty boundaries there are unchanged; only the *connect* step is.

Chosen behavior (the user's two rulings on the forks):
1. **Automatic, always** — not a dismissible per-wallet variant. Watch a wallet → its Peer fills and Privacy Pools deposits land. Unwatch it → its cursors/pending leave (already wired per-address in `WalletStore`). No switch anywhere. The reads are keyless public-chain reads that already ride `WalletIngest.refresh`, so the marginal cost is ~nil.
2. **Catalog tiles stay** — "that is where a user may specifically go to add a wallet and it helps with discovery." So the Peer / 0xBow Privacy Pools tiles and screens remain, but retooled: no toggle. With wallets watched, the screen states the fact ("Watching N wallets") and doors to the wallet manager; with none, it's the "Watch a wallet" invitation — the add-a-wallet entry the tile exists to be. The catalog seat reads **connected whenever ≥1 wallet is watched** (honesty: a live reader must not show "connect"), reconciled centrally by `BridgeStore.reconcileWalletSeats()` — called on watch/unwatch, on each screen's appear, and once per foreground in `BridgeRefresh`.
3. **Show in the address book** — a watched wallet's own Peer fills and Privacy Pools deposits fold into its `AddressCard` "history" (shared `addressHistory(for:in:)`, joined on `Thing.walletAddress`, the owner both bridges already stamp). The wallet's card is now the one place its riding-read activity lives, so a seat with no screen of its own still has a home.

Also this session (a separate, approved cleanup): **`CombinedWalletsSheet` became a `DSTray`** — it was the last wallet feed tray still hand-rolling its own `NavigationStack` + navigationTitle + Done toolbar while "Where it's held" and "Worth a look" already rode `DSTray`. Design law: trays are never hand-rolled. Grabber dismisses now, like its siblings.
...
Resolution — kill the separate surface, fold its unique content inline:
- **`WalletBalanceHeadline.onOpen` → nil.** The number is a read, not a door; no chevron.
- **New inline `eachWalletSection`** in the `.wallet` feed shape, between the headline and the holdings treemap. Shown only when **>1 wallet is watched AND the feed is unscoped** (scoped, the whole feed already *is* that one wallet), and only for wallets with the ≥2 aligned samples a line needs — the exact set the sheet's "Each wallet" list drew. Each row (face · name · value · sparkline · delta pill) **taps to scope the feed to that wallet**, the same move the switcher bar makes — so the rows are also navigation, not just a display.
- **What happened to the sheet's other parts:** the duplicated hero is gone (the feed's headline already is it); **"What moved"** already exists on the feed as the headline's `moverLine()` ("Mostly ETH · +$310"); **"Made of"** (a stacked-area composition chart) retired — the per-wallet rows *are* the composition, and it was the two-charts-competing offender.
...
THE RULE: a paragraph holds at most two sentences; when a summary needs a third, a blank line (`\n\n`) starts a new paragraph. The break falls on the BEAT, not mechanically after every second sentence — the usual shape is "what lands" / "how it connects + the read-only promise", and a beat that spans two sentences (the safety promise "No account, no key, read-only: nothing here…", an honest caveat like Privacy's key-scope note) stays whole rather than being split to hit the count. A two-sentence hook stays a single paragraph, untouched.
...
Three mockups were made (`design/wallet-cashapp/wallet-cashapp-mocks.html`): **A** un-boxed everything into one hero plus one list (the pure borrow); **B** kept the boxes and cut them from six to three; **C** rebuilt the manager. **B + C shipped.** A was refused because it reverses §160 ("i like both boxed"), and §160's own argument — parcels beat strata when the eye is scanning for sections — was right about six sections. The fix was to stop having six, not to un-box the one that leads. Un-boxing later is one deleted modifier if the boxed version still reads heavy on device.

**What shipped, feed side:**
- **One card holds the balance, the per-wallet split, and the security read.** They were never three subjects: "what's it worth", "whose is it", "is it okay" are the questions of a single glance at a single number. `WalletWarningsLine` → `WalletWarningsStrip`, a faint-fill row on the card's own bottom edge (the §196 badge capsules retire; their counts come back as words via `WalletWatch.summary`, the same shared per-kind tally the badges read). `eachWalletSection` → `WalletFaceChips`, a scrolling row of face + value + delta, same ≥2-sample guard, same tap-to-scope. The 80pt per-wallet sparkline is the one deliberate loss: at that width it was decoration, and a tap now scopes the feed where the same line draws full-width as the room's headline.
- **Aave + Morpho → one `WalletLendingCard`, two rows.** They were never two subjects, just two providers of one. Collateral moves behind the row (it's the denominator of the health factor, which is already stated); the money leads on the trailing edge and health rides the subline as a sentence ("Health 1.82 · Base"). Still no chevron on either — acting happens on Aave or Morpho.
- Six cards → **two** (balance, holdings) plus the conditional lending card.

**The room's one row shape**, new in `Screens/WalletRow.swift`: a 34pt mark (SF Symbol / `WalletFace` / protocol monogram / a thing's own `KindGlyph`), a title, a gray subline, and a trailing value (`WalletRowValue`) or chevron (`WalletRowChevron`). `WalletSectionLabel` is the section name, with an optional count-link. Rank comes from position under a small gray label now, not from owning a surface. Applied to the lending rows, the warnings strip, `TokenQuickSheet`'s "Held in", `WalletHistoryScreen`'s transactions, and the manager's address book. The feed's own transaction rows are deliberately untouched — those are the generic feed row shared with every source, not a wallet component.

**The type rule, and the part of the borrow that was refused.** The pass wanted the rounded money face on every row title (Cash App's real tell: "tappable" registering before the words are read). **Refused** — §190 already ruled that exact question in the conversation that produced the slab: the complaint was *"we have different fonts"*, so the fix was ONE font and it's SF Pro Text. Rounded row titles beside SF Pro slab labels hands the complaint straight back. So `DSTextStyle.rowTitle17` is SF Pro semibold and **weight** carries the tappable signal; rounded stays where §190 and the 2026-07-09 rule left it — display headings and the money rungs.

**`WalletHistoryScreen`:** `dsListCardRow` dropped. The room's longest list gave all 411 transactions an opaque surface and a shadow each, where the day header was already doing the grouping. Row insets go with the card — a List sizes its defaults for a card's body, and keeping them over a bare row left ~95pt of air per transaction (caught on the sim, fixed same pass).

**The manager (mock C).** It had grown to **nine blocks**: shelf, count, field, a "Just name it" text button, Connect, the peek chip, the promise, a Connection door, status rows, then a headed address-book section with its own sort menu and a two-sentence footer. Now: shelf → one note line → field → Connect → one sentence → the book AS the page's list → Connection at the foot.
- **`DSSlabField` gains a second verb** (`secondaryLabel`/`secondaryArmed`/`secondaryAction`). "NAME" lives inside the field slab, armed only over a real addable address. Two verbs in one slab is still one control for one act; a second line was two.
- **The roster slots wear their wallet's value**, and the shelf note reads "2 of 5 watched · $45K" and absorbs the syncing spinner — the one fact the roster was missing, and a spinner belongs on the thing being read. Fed by the `topHoldingsByWallet` call `sync` already made and threw away.
- **The book's footer paragraph is gone.** Tapping a star visibly drops the wallet into the shelf at the top of the same screen, which teaches the star better than two sentences about it; "one gray sentence per screen" (§190) was already spent on the read-only promise.
- **Copy leaves the trailing slot** for the row's context menu, beside Remove — two buttons competing there made the star, which is the real verb, look like one of a pair. Swipe-to-remove retires with it, squaring the row with the design law's own reading (swipe verbs are reads).
- **The peek chip stays** (mock C wanted it demoted to placeholder text): it only ever renders on an empty roster, so it costs nothing in the normal case, and turning a one-tap demo into a string you must type by hand is a real loss exactly when a new person has the least patience.

Verified on sim (two watched wallets, seeded histories, dark): the one card renders balance + chips + strip; the lending card renders nothing with no positions (correct); the history page and the book both read as flat lists. Build green.

**Found, then fixed — the address book held a watched wallet twice** (2026-07-25). `AddressBook.key(for:)` keyed "vitalik.eth" and `0xd8dA…6045` as different entries, so watching by ENS named one entry under the ENS string while chain reads named another under the hex. The list showed both, and the hex one wore an empty star while the wallet was plainly watched. Pre-existing, but the redesign made the book the page's main content, so it went from invisible to the first thing you see.

The ruling behind the fix: **a name is not an identity.** The book keys on the ADDRESS a spelling stands for, resolving through the same `wallet.resolutions` cache `WalletStore` already fills, and the person's own words survive as the entry's *name*. `reconcileAliases` re-keys and merges whenever a resolution lands and once at init, so an existing book heals on the next launch — no sync, no migration flag. Merge rule: the address-keyed row stands and keeps its name unless that name is only the address's own short form (the unnamed fallback), in which case the name given to the spelling wins; earliest `addedAt` survives; kind and provenance fill in from either side rather than leaving with the row that goes.

**A delegated wallet is still a wallet** (same day, found by the fix's own probe: vitalik.eth came back `kind=contract`). Since Pectra an ordinary EOA can carry an EIP-7702 delegation — `0xef0100` plus a delegate address — as its `eth_getCode` answer, and `AddressKind` read any non-zero code as bytecode. So a person's own account was labelled "Contract" and lost its identicon face, which is the one thing the mark exists to say: a wallet is a WHO. `AddressKind.hasCode` now excludes the delegation shape (reusing `WalletSafety`'s existing decoder rather than writing a second one) and keeps asking the other chains, so a genuine deployment elsewhere still answers contract. A cached verdict is never revisited — `detectPending` only asks about `.unknown` — so a one-time pass drops every stored `.contract` back to unknown and lets the corrected rule re-answer; that costs nothing visible, since an unchecked row and a wallet row look identical. `hasCode` also returns nil (unknown) when NO chain answered, which its own doc always claimed but the code didn't do: a total outage used to cache "wallet". Verified against both cases — `0xd8dA…6045` now reads wallet, the Uniswap router still reads contract.
...
**Volume is not news — so two modules were deleted, not restyled.** A person receiving dozens of things a day does not care how many arrived. `sourceMix` ("What landed: Bluesky 6, Photos 4, RSS 3") and `hourStrip` ("When it landed") are both compositions of *how much*; neither survives the amendment, because there is no arrangement of a tally that makes the tally interesting. The reading RECORD went with them ("most reading to land in one day this month — 12 so far"): a tally wearing a surprise, and in a deluge the monthly maximum falls constantly, so it was neither rare nor news. The wallet record stays — it measures a MOVE, not an arrival.

The doctrine in `TodayBrief`'s own header is amended to match: the rule is not "no numbers" — "2 more overdue" is a STATE, and money moving is still the one place a count IS the event — it is **no numbers about how much arrived**.

**The money is the crown.** The synthesis card used to open the screen, which made the agent's read outrank the thing that actually changed while you slept. Order is now hero → pair (watchlist, glued to it per §166's 2026-07-23 ruling) → themes → synthesis → the leads. Without a watched wallet the crown falls through to themes — the same screen minus a section, because money leads when it's yours and it moved, not because the app prefers the subject.

**The wallet attribution moves out of the notes and under the number** (`MoneyHero` arg 11): "Up $1,247 today. ETH did the lifting." Two facts the total and its pill cannot carry — the magnitude in money (a percentage hides whether +5.2% is a coffee or a month's rent) and which holding did it (the day-scoped `holdingsDeltas`, never the all-time one). Both halves fail independently and silently. It was a synthesis note before; an attribution belongs under the number it attributes, not in a card three modules down.

**The Themes treemap takes the vacated slot** — `TagMap("What you're into", …, "plain")`, the same map the All feed leads with, and the user's own call that it outranks both modules it replaced. Three things make it the brief's map rather than the feed's:
- **It reads a month, not the day.** A theme is a shape that forms over time; scoped to the window it would just be today's tags — the volume read wearing better clothes.
- **The cells print no count.** New `iconMode == "plain"` in `GenTagMap` suppresses the "N things" line. Area already encodes magnitude; the number was the same fact twice, and it was the half nobody wanted. The count still rides the ref — it sizes the cell.
- **Its subline is the one thing the map can't draw: what's new.** Deliberately NOT "new this week" (user, same session: a week means nothing in a deluge). The only claim we can keep is that a theme did not exist before today, so a theme is new when its OLDEST thing landed inside the brief's own window, and the line NAMES them ("Foldables is new", "Foldables and Recipes are new"), capped at three so it stays a sentence. Nothing new, no subline — never padded.

Verified on sim (dark, seeded corpus + wallet history): `-todayProbe` composes `Stack([hero, pair, themes])` with the attribution line present and no `mix`/`hours`; the rendered screen shows $45K +5.2% with "Up $1,247 today. ETH did the lifting." under it, and a six-cell themes map with no tallies. Build green.

**Not done, deliberately.** The whisper's own digest (`DayBrief.detail`, §165) still leads with "121 new" — the same arrival count this ruling just outlawed, and it rides the kept pill's trailing signal too ("Catch me up · 121 new"). It is a different surface with its own ruling, so it was left alone rather than changed unasked; it is the next domino. The All feed's Themes map also keeps its "N things" cells — the "plain" mode exists now, but pointing it there is a Feed decision, not this one.

**Amendment, same day — the sentence goes ABOVE the wallet** (user: "yea i think that line should be above wallet like you had suggested"). The attribution shipped as the hero's own subtitle a few hours earlier; it is now its own module, `DayLede`, the first thing in the document and the first thing on the screen, in `heading22` display type with no surface — a sentence on the page, not a card. Under the number it read as a caption on a chart; above it, it is the day's headline, and the number gets the room to be a number in. Wallet-only for now (no move, no lede — nothing is padded to fill the slot), so `MoneyHero` loses arg 11 again and goes back to eleven arguments.
