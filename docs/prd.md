...
**Found, not fixed — the address book holds a watched wallet twice.** `AddressBook.key(for:)` keys "vitalik.eth" and `0xd8dA…6045` as different entries, so watching by ENS names one entry under the ENS string while chain reads name another under the hex. The list shows both, and the hex one wears an empty star while the wallet is plainly watched. Pre-existing (nothing in this pass touched `AddressBook` or `isWatched`), but the redesign makes the book the page's main content, so it is far more visible than it was. Wants a resolve-then-key fix.
