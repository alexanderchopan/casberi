...
- **Grouping — a source's day reads together, still by recency.** In the All shape, 3+ bundleable things from one source in one day form a VISIBLE cluster: a light source header, its newest few rows inline (cap 4), then a "N more" tail that opens the source's shape. The cluster sits at its newest member's slot, so the day's top-level order still flows by recency — grouping, never ranking; the day header keeps the true total. (Evolved 2026-07-12, user: first lowered the old collapse-to-count bundle 4 → 3, then — with a feed-heavy corpus where music/events/social dominate — replaced collapse with a visible cluster: a source's items want to read together and be *seen*, not hidden behind a count nor scattered through a strict chronological interleave. Full reorder-by-source was still rejected: placing each cluster at its newest member keeps the recency flow without a "which source leads" ranking question.) Human capture kinds never bundle (screenshot, voice, approval, anything from You) — each is one deliberate act; machine bulk (transactions, synced articles) is one act producing many rows. The day header keeps the true total. No ranking, no algorithmic feed — the Reminders "Older" collapse, applied to arrival volume. Single-source shapes never bundle (the shape IS the source).
