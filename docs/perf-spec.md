...
1. ~~`ThingShareLink`'s body runs `Capture.detectURL`~~ (landed 2026-09-06 as `ShareTargetMemo`, and the whole menu is lazy since §661) — it runs `Capture.detectURL` → an `NSDataDetector` over `thing.content`, inside the row's non-escaping `.contextMenu`. **This contradicts the §260 amendment two lines above it in `FeedScreen.swift`**, which states all three detector scans moved out; one came back.
