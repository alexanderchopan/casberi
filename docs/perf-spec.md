...
**Two landed 2026-09-09 (prd §661), on a report rather than a sample.** The context-menu derivation (item 1 and the "Verb derivation" bullet below) is DEFERRED, not memoised: `RowVerbMenu` is a View, so the derivation runs when the press raises the menu. `RowEntrance` now shows a row met by scrolling at rest — the stagger sized for the first screen was making every scrolled-in row invisible for a third of a second. Both are removals, so both ship without a number.

**Two more landed 2026-09-21, and three of the six below were already fixed when
this list was re-read.** The list had gone stale in both directions — worth
checking each item against the tree before costing any of it.

1. ~~`ThingShareLink`'s body runs `Capture.detectURL`~~ — LANDED 2026-09-06 as `ShareTargetMemo`, and the whole menu is lazy since §661.
2. ~~`NotifySweep.classify(thing, now:)` twice per row~~ — **LANDED 2026-09-21.** `ThingRow.liveBody` now binds `isNew` and `isAlarm` once, the way it already bound `project` and `leader`, and `isAlarmClass`/`timeInk` became FUNCTIONS taking those facts rather than computed properties reading them. Measured by reading, as before: it was six `SharedStore.groupDefaults` reads and two classifier passes per row per body evaluation (`timeInk` read both, the weight and the animation read `newSinceLastSeen` twice more, the spoken label read both again). Now one of each. The shape is the guard — a fourth reader cannot appear without a caller supplying the bools — and `row-cost-audit.py` pins both spellings with two mutations.
3. ~~`DS.legibleInk` → `solveInk`~~ — LANDED 2026-09-06 as `AppIconTile.inkMemo`, keyed on the page background (see that check's comment for the key that was wrong first).
4. `ThingVoice.rowLabel` builds localized strings per row **whether or not any assistive technology is running** (`.accessibilityLabel` is not autoclosure'd), and `LiveTimeText`'s own `Self.spoken(date)` label is then discarded by the parent's `.accessibilityElement(children: .ignore)`. **DECLINED 2026-09-21, and this is the reason rather than an oversight.** The only gate available is `UIAccessibility.isVoiceOverRunning`; iOS publishes no "any AT running" flag, and **Voice Control has no flag at all** while depending on these labels to name an interactive row. So the gate would silently break Voice Control to save, in the common path, about three localized lookups and a small array — the worst trade in this list. Removing `LiveTimeText`'s discarded label is a separate, smaller win and needs all 19 of its call sites checked for `children: .ignore` first; not done.
5. Two `.task(id:)` keys run a SQL `COUNT` on **every body pass** — `headIdentity` (plus a 16-element array build and `joined`) and the All room's `corpusRevision`. **REFRAMED 2026-09-21, NOT attempted, and the ranking above ("likely the biggest remaining") predates its own fix.** `#Index<Thing>` landed as P5.1, so both counts are now served off an index b-tree rather than a table scan. What is left is one redundant COUNT per All-room body pass (`corpusRevision` and `headIdentity` ask the identical question there), a 16-element array build, and the fact that **every page the pager has ever built re-evaluates these keys on every graph update** — the multiplication `safetyNetKey`'s own guard was written for. The guard shape that would close it is known and is `safetyNetKey`'s: put the room's served-ness in the key, so an off-screen page pays nothing and crossing the boundary still restarts the task. **It is not being applied blind.** `headIdentity` carries three separately-documented shipped incidents of the form "a black room, permanently, because nothing static can see a memo that never invalidates", and there is no measurement saying what the win is. Measure first: the honest instrument is a count of key evaluations per foreground sweep, which the Diagnostics screen (prd §623) is the right place for.
6. ~~`NumberFormatter` / `DateFormatter` constructed per call~~ — **LANDED 2026-09-21.** A survey found 21 body-reachable sites (not ~15), 14 with constant configuration and 7 whose configuration varies. Fixed: `HegotaFormat` (five `NumberFormatter`s and two `DateFormatter`s, reached from ~14 body sites in its own file plus the feed's row), `FramesFormat.stamp` (built BOTH kinds per call), `FramesMoney` (`fee`/`signedETH`/`eth(places:)`), `DevnetTokens.quantity`, `PrivacyPoolsRoom.roundedSet` (reached three times to build one sentence), `WalletApprovalExposure.number`, `WalletApprovalAge.text`, `WalletIngest.format`, `BitcoinBridge.formatAmount`, `ThingContent.productPrice`. `PrivacyDevnetFigures.grouped` was DELETED — zero callers, and §605 had already folded it into `DSCount`.

   **Three rules came out of it, and the first cost a red harness.** (a) A file compiled Foundation-only against stubs by a harness — `PrivacyPoolsRoom`, `FramesReading`, `WalletApprovalExposure` — **may not reach `Design/DSCount`**; it keeps its own static and says why. The first cut routed two of them through the shared helper, and `wallet-rooms-selftest.sh` and `frames-tx-selftest.sh` failed on it immediately. (b) A varying precision is spelled as the two or three shapes the call sites actually use, with a default arm that stays CORRECT for a fourth, rather than as a mutable dictionary needing a lock or an actor. (c) The one genuinely keyed cache (`productPrice`, keyed on a currency code off the record) is `@MainActor`, because it is written on fill where none of the others is. `frames-tx-selftest.sh`'s six-places mutation was re-anchored onto the new spelling rather than deleted — it still asserts the same ruling.
...
| P3 per-row costs | **Seven fixed 2026-09-06 (prd §626)**, plus §628's four fetch-in-body sites and the Keychain memo, including a quadratic, and pinned by `row-cost-audit.py`. Of the six then listed open: **three were already fixed** when the list was re-read 2026-09-21, **two landed that day** (the row's doubled `NotifySweep.classify`, and 11 per-call formatter sites across the devnet and wallet rooms), **one is declined with its reason** (the spoken row label — the only gate available breaks Voice Control) and **one is reframed and deliberately not attempted** (the `.task(id:)` COUNTs: `#Index` has since made them index-backed, and `headIdentity` carries three shipped "black room" incidents, so it needs a number before it needs a patch). Still nothing measured — there is no scroll instrument and none is planned, which is why every item here was found by reading. |
