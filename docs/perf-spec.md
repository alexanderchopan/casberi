...
**Two landed 2026-09-09 (prd §660), on a report rather than a sample.** The context-menu derivation (item 1 and the "Verb derivation" bullet below) is DEFERRED, not memoised: `RowVerbMenu` is a View, so the derivation runs when the press raises the menu. `RowEntrance` now shows a row met by scrolling at rest — the stagger sized for the first screen was making every scrolled-in row invisible for a third of a second. Both are removals, so both ship without a number.

**Still open from that same read, worst first** — each runs per row per body evaluation unless noted:

1. ~~`ThingShareLink`'s body runs `Capture.detectURL`~~ (landed 2026-09-06 as `ShareTargetMemo`, and the whole menu is lazy since §660) — it runs `Capture.detectURL` → an `NSDataDetector` over `thing.content`, inside the row's non-escaping `.contextMenu`. **This contradicts the §260 amendment two lines above it in `FeedScreen.swift`**, which states all three detector scans moved out; one came back.
