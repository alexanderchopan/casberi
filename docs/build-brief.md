...
Tokens port from `src/index.css` to a Swift token layer. Components hold zero raw hex. Surfaces: #000 page, one sheet token `--ds-surface-sheet` (#111113 dark, #fff light) for cards, tiles, trays; washes dead. Tint: systemBlue dark #0A84FF, one-line swap; pink one line away. Color rule: identity, state, or magnitude; decoration banned; magnitude = tint at opacity scaled by count. Text ramp: white / 60% / 30%. No hairlines: rows separate by spacing and press fills, groups by air; nothing draws a line — zero exceptions (prd §39, 2026-07-10: the Apps page's connected strip and its `fillLine` divider died; the catalog is one grid). No plates either (prd §749–§759, §782, superseding §61's elevation ladder): a row, a block, a list section or a stat stands on the page, and `dsListRow` draws no background. A well (`dsWell`) holds only what you type into, a room's lead, a monospaced value, or a small control face — `plate-audit.py` lists every file allowed one. Headers are sentence case — no ALL-CAPS eyebrows, no `.kerning()`. SF ramp: 34/22/18/16/14/12/10 (the reading band 17/15/13 rose to 18/16/14 with more line-height — prd §206, 2026-07-25; display tier is SF Rounded, functional text SF Pro — 2026-07-09). Spacing scale (`DS.Space`) 4/10/14/18/24/32 — the three content-rhythm rungs raised for breathing room, atomic and macro rungs held (prd §206). Radii: cards 10, sheets 16, app icons 22.37%. Motion: 250ms, Apple sheet curve, one animation per moment; hero morph tile→detail uses matchedGeometryEffect. Copy: Bob's words — no sync, scope, token, connector, OAuth where he sees it (the token-bridge setup screens name "token" deliberately — an Alice-facing power surface, not where Bob sees it).
