...
**Status: BUILT, UNSIGNED (2026-08-21). Steps 1–8 of §10 are done; step 9 is
not.** This file is kept as WRITTEN, append-only like the ledger — read
**prd §426** for what the build changed, which is: a sixth fixture (the five
below cannot catch a `safeTxGas`/`baseGas` transposition, and that mutation
survived the harness's first run), `.privateKeyUsage` dropped from §4 with a
reason, the curve library pinned to 0.21.1 rather than the newest tag, the
confirmations path in §7 corrected against the service's own source, and
Gnosis Chain refused rather than signed on. Every constant in §3 survived
unchanged.

The original status line, true when written: *SPEC ONLY. No Swift written,
nothing compiled, nothing measured.* Authored on Linux with no Xcode and no
Swift toolchain. The one thing here that IS verified is the cryptographic
constants — see §3, they are computed by
`scripts/support/safetx-vectors.py`, not recalled.
