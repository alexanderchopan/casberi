// Stripe promo — CONSUMER cut. Grounded line-by-line in
// Model/StripeBridge.swift + its catalog offer (2026-07-31):
//   0 — five shapes land, and they're the ones money MOVING makes:
//     dispute opened, dispute closed, payout, subscription canceled,
//     recurring payment failed. Titles below are the real ones the file
//     builds ("Dispute opened · $420.00 — evidence due …", "$2,140.00 paid
//     out to your bank", "Payout failed · … — <Stripe's own reason>").
//   1 — the hero: a dispute carries `evidence_details.due_by`, landed as a
//     reconciling `dueAt` (the ENSExpiry/Aerodrome shape), so it rides
//     "what's next" instead of an inbox. The one row here with a clock, and
//     the reason the bridge exists.
//   2 — the restraint IS the feature: individual charges NEVER land. "A
//     payment is a tally, and your dashboard already counts those."
//     Never a customer's details either.
//   3 — read-only STRUCTURALLY (a restricted key with read scopes cannot
//     refund, charge, pay out or write), and test-mode keys are REFUSED at
//     connect so pretend money never files beside real money.
// Deterministic (stills) -> render frame-by-frame:
//   node render.js clip-stripe-editorial.html --size=1080x1920
const fs = require('fs'), path = require('path');

const STRIPE = '#635BFF', INK = '#14110d', GREEN = '#3fb950', ORANGE = '#FF6B4A';
const BEATS = [
  { kick: 'NEW',          head: 'What your money\ndid today.',        accent: STRIPE, tag: 'NEW' },
  { kick: 'THE CLOCK',    head: 'A dispute has\na deadline.',         accent: ORANGE, tag: null },
  { kick: 'NOT EVERY CHARGE', head: 'Your dashboard\nalready counts.', accent: INK,   tag: null },
  { kick: 'READ-ONLY',    head: "It can't touch\nyour money.",        accent: GREEN,  tag: null },
];
...
/* 0 — the five rows that land */
.row{display:flex;align-items:center;gap:16px;padding:13px 0;will-change:opacity,transform;}
.tag{font-size:15px;font-weight:800;padding:7px 13px;border-radius:100px;flex:none;letter-spacing:.03em;min-width:104px;text-align:center;}
.rowtxt{font-size:22px;font-weight:640;line-height:1.3;}

/* 1 — the deadline */
.discard{background:rgba(255,107,74,.1);border:2px solid rgba(255,107,74,.32);border-radius:22px;padding:28px;will-change:opacity,transform;}
.discard .t{font-size:26px;font-weight:750;line-height:1.35;}
.discard .t b{color:${ORANGE};}
.upnext{margin-top:24px;background:rgba(255,255,255,.06);border-radius:18px;padding:20px 24px;will-change:opacity,transform;}
.upnext .l{font-size:17px;font-weight:750;letter-spacing:.08em;color:rgba(255,255,255,.4);margin-bottom:10px;}
.upnext .i{font-size:22px;font-weight:680;}
.upnext .i span{color:${ORANGE};}
.missnote{margin-top:24px;text-align:center;font-size:22px;font-weight:700;color:rgba(255,255,255,.6);will-change:opacity;}
...
.whynote{margin-top:26px;text-align:center;font-size:23px;font-weight:700;color:${STRIPE};line-height:1.4;will-change:opacity;}
...
.vrow s{width:46px;height:46px;border-radius:50%;flex:none;background:rgba(255,107,74,.16);color:${ORANGE};display:flex;align-items:center;justify-content:center;font-size:22px;text-decoration:none;font-weight:700;}
...
.outro .u{font-size:34px;letter-spacing:.1em;color:#14110d;margin-top:40px;font-weight:600;} .outro .u b{color:${STRIPE};}
</style></head><body>
<div class="stage">
  <div class="grain"></div>
  <div class="mast mono"><span>CASBERI</span><span>STRIPE</span></div>
  <div class="rule" id="rule"></div>
  <div class="wm" id="wm">01</div>

  <div class="comp" id="comp0">
    <div class="shd mono">FIVE THINGS LAND</div>
    <div class="row" data-i="0"><span class="tag" style="background:rgba(255,107,74,.18);color:${ORANGE}">DISPUTE</span><span class="rowtxt">Dispute opened · $420.00</span></div>
    <div class="row" data-i="1"><span class="tag" style="background:rgba(63,185,80,.18);color:${GREEN}">PAYOUT</span><span class="rowtxt">$2,140.00 paid out to your bank</span></div>
    <div class="row" data-i="2"><span class="tag" style="background:rgba(255,107,74,.18);color:${ORANGE}">PAYOUT</span><span class="rowtxt">Payout failed · account closed</span></div>
    <div class="row" data-i="3"><span class="tag" style="background:rgba(99,91,255,.24);color:#a8a2ff">CHURN</span><span class="rowtxt">A subscription canceled</span></div>
    <div class="row" data-i="4"><span class="tag" style="background:rgba(99,91,255,.24);color:#a8a2ff">DUNNING</span><span class="rowtxt">Payment failed — retries Friday</span></div>
  </div>

  <div class="comp" id="comp1">
    <div class="shd mono">THE ONE ROW WITH A CLOCK</div>
    <div class="discard" id="discard">
      <div class="t">Dispute opened · $420.00 — <b>evidence due Aug 7</b></div>
    </div>
    <div class="upnext" id="upnext">
      <div class="l mono">COMING UP</div>
      <div class="i">Dispute evidence · <span>due in 6 days</span></div>
    </div>
    <div class="missnote" id="missnote">Miss it and the money's just gone.</div>
  </div>

  <div class="comp" id="comp2">
    <div class="shd mono">WHAT NEVER LANDS</div>
    <div class="nope" data-i="0"><s>✕</s> Every individual charge</div>
    <div class="nope" data-i="1"><s>✕</s> Your customers' details</div>
    <div class="whynote" id="whynote">A payment is a tally.<br>Your dashboard already counts those.</div>
  </div>

  <div class="comp" id="comp3">
    <div class="shd mono">A RESTRICTED KEY, READ SCOPES ONLY</div>
    <div class="vrow" data-i="0"><s>✕</s> Can't refund</div>
    <div class="vrow" data-i="1"><s>✕</s> Can't charge</div>
    <div class="vrow" data-i="2"><s>✕</s> Can't pay out</div>
    <div class="vgood" id="vgood"><i>✓</i> Test keys are refused — only real money.</div>
  </div>
...
window.seek=function(t){
  ...
  if(TAGS[active]){
    ...
    tag.style.background='${STRIPE}';tag.style.color='#fff';
...
function animateComp(i,p,t,local){
  if(i===0){
    stag('#comp0 .row',p,0.12,0.24,18);
  } else if(i===1){
    pop(document.getElementById('discard'),p,0.03,0.28,22);
    pop(document.getElementById('upnext'),p,0.36,0.28,20);
    document.getElementById('missnote').style.opacity=clamp01((p-0.68)/0.24);
  } else if(i===2){
    stag('#comp2 .nope',p,0.22,0.3,20);
    document.getElementById('whynote').style.opacity=clamp01((p-0.56)/0.28);
  } else if(i===3){
...
fs.writeFileSync(path.join(__dirname,'clip-stripe-editorial.html'),html);
console.log('wrote clip-stripe-editorial.html',(html.length/1024).toFixed(0)+'KB',TOTAL.toFixed(1)+'s');
