// Apple Wallet launch clip (prd §313/§317) — FinanceKit.
//
// The house template is lifted verbatim from build-bridges-batch.js so the
// grammar stays identical across the family.
//
...
/* ============================ APPLE WALLET ============================= */
// Every line below is grounded in the shipped source:
//   AppleWalletBridge.rowTitle   "<merchant> · <money>"
//                                "Refunded · <merchant> · <money>"
//                                "Pending · <merchant> · <money>"
//   AppleWalletRoom.headlineText "$X this month · most of it at Y"
//   AppleWalletRoom.sublineText  "Netflix went up 16% — $15.49 to $17.99"
//                                "Spotify hasn't charged you in 29 days"
//   AppleWalletScreen            the four promises, verbatim in spirit
const GRAPHITE = '#1d1d1f';
clip({
  name: 'applewallet', mast: 'APPLE WALLET', accent: GRAPHITE,
  beats: [
    { kick: 'THE NAME',   head: 'Every other card\nsees a number.',        accent: ORANGE, tag: 'NEW' },
    { kick: 'THE DELTA',  head: 'Your bank says $17.99.\nIt was $15.49.',  accent: INK },
    { kick: 'THE SILENCE', head: "The one that stopped\ncharging you.",    accent: INK },
    { kick: 'ON DEVICE',  head: 'No server.\nNothing to upload.',          accent: GREEN },
  ],
  comps: `
  <div class="comp" id="comp0">
    <div class="shd mono">WHAT EVERY OTHER CARD CAN SEE</div>
    <div class="cnt" data-i="0"><span class="l">0.0312 ETH &nbsp;·&nbsp; an address</span><span class="r">ONCHAIN</span></div>
    <div class="cnt" data-i="1"><span class="l">$6.50 &nbsp;·&nbsp; a card descriptor</span><span class="r">VIRTUAL</span></div>
    <div class="cnt" data-i="2"><span class="l">£4.20 &nbsp;·&nbsp; a moment</span><span class="r">GNOSIS PAY</span></div>
    <div class="keep" id="keep0"><span class="l">Blue Bottle Coffee · $6.50</span><span class="r">APPLE CARD</span></div>
    <div class="note" id="note0">The merchant's real name never touches a chain.<br><em>This is the only source in the app that knows it.</em></div>
  </div>
  <div class="comp" id="comp1">
    <div class="shd mono">THE ONE NOTHING ELSE TELLS YOU</div>
    <div class="creep" id="creep1">
      <div class="cline"><span class="was">$15.49</span><span class="arrow">→</span><span class="now">$17.99</span></div>
      <div class="cwho">Netflix went up 16%</div>
    </div>
    <div class="note" id="note1">Your bank pushed you the charge. Nothing pushed you<br><em>the change.</em> Three charges make a cadence; the fourth is news.</div>
  </div>
  <div class="comp" id="comp2">
    <div class="shd mono">WHO YOU ACTUALLY PAY</div>
    <div class="bar" id="b0"><div class="bl"><span>Whole Foods</span><span>$412</span></div><div class="btrack"><i style="width:78%"></i></div></div>
    <div class="bar" id="b1"><div class="bl"><span>Blue Bottle Coffee</span><span>$96</span></div><div class="btrack"><i style="width:31%"></i></div></div>
    <div class="bar" id="b2"><div class="bl"><span>Amazon</span><span>$43</span></div><div class="btrack"><i style="width:14%"></i></div></div>
    <div class="row" id="quiet2"><span class="pill" style="background:rgba(255,159,10,.18);color:${AMBER}">Quiet</span><div><div class="rt">Spotify hasn't charged you in 29 days</div><div class="rs">Every ~31 days, until it wasn't</div></div></div>
    <div class="note" id="note2">Ranked by what you spent, not how often you tapped.</div>
  </div>
  <div class="comp" id="comp3">
    <div class="shd mono">BEFORE YOU CONNECT</div>
    <div class="cnt" data-j="0"><span class="l">Uploaded anywhere</span><span class="r">NEVER</span></div>
    <div class="cnt" data-j="1"><span class="l">Sold, or used for ads</span><span class="r">NEVER</span></div>
    <div class="cnt" data-j="2"><span class="l">Able to spend or move money</span><span class="r">NEVER</span></div>
    <div class="keep" id="keep3"><span class="l">Disconnect, and it's all deleted</span><span class="r">ONE TAP</span></div>
    <div class="note" id="note3">Read on your iPhone, stored on your iPhone.<br><em>Casberi has no server to send it to.</em></div>
  </div>`,
  css: `
.creep{background:rgba(255,255,255,.06);border-radius:26px;padding:44px 32px;text-align:center;will-change:opacity,transform;}
.cline{display:flex;align-items:baseline;justify-content:center;gap:24px;font-family:ui-monospace,monospace;}
.cline .was{font-size:52px;font-weight:800;color:rgba(255,255,255,.34);text-decoration:line-through;text-decoration-thickness:4px;}
.cline .arrow{font-size:38px;color:rgba(255,255,255,.4);}
.cline .now{font-size:76px;font-weight:800;color:${ORANGE};letter-spacing:-.02em;}
.cwho{margin-top:22px;font-size:29px;font-weight:750;}
.bar{margin-bottom:26px;will-change:opacity,transform;}
.bl{display:flex;justify-content:space-between;font-size:25px;font-weight:750;margin-bottom:12px;}
.bl span:last-child{font-family:ui-monospace,monospace;color:rgba(255,255,255,.66);}
.btrack{height:14px;border-radius:100px;background:rgba(255,255,255,.08);overflow:hidden;}
.btrack i{display:block;height:100%;border-radius:100px;background:#fff;transform-origin:left;will-change:transform;}
#quiet2{margin-top:34px;}`,
  anim: `
  if(i===0){stag('#comp0 .cnt',p,0.13,0.24,16);pop(document.getElementById('keep0'),p,0.52,0.26,20);show('note0',p,0.76);}
  else if(i===1){pop(document.getElementById('creep1'),p,0.04,0.32,26);show('note1',p,0.62);}
  else if(i===2){
    ['b0','b1','b2'].forEach((id,k)=>{const el=document.getElementById(id);const rp=clamp01((p-k*0.12)/0.3);
      el.style.opacity=rp;el.style.transform='translateY('+((1-back(rp))*18)+'px)';
      const fill=el.querySelector('i');fill.style.transform='scaleX('+easeOut(clamp01((p-0.08-k*0.12)/0.42))+')';});
    pop(document.getElementById('quiet2'),p,0.56,0.28,20);show('note2',p,0.8);}
  else if(i===3){stag('#comp3 .cnt',p,0.13,0.24,16);pop(document.getElementById('keep3'),p,0.52,0.26,20);show('note3',p,0.76);}`,
});
