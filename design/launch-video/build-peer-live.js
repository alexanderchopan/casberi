// Peer promo — live-animated. node render.js clip-peer-live.html --size=1080x1920
// Grounded in Model/PeerBridge.swift + Screens/PeerScreen.swift (origin/main, prd §113):
//   Peer (peer.xyz, formerly ZKP2P) — fiat↔crypto trades that settle ONCHAIN into your OWN wallet
//   NO account, no key, no OAuth — the seat rides the watched Wallet (like Strava rides Health)
//   title format: "Bought 25 USDC with Venmo on Peer"; degrades honestly:
//     unknown token -> "Bought crypto on Peer"; unknown platform -> "Bought USDC on Peer"
//   CAPTURE ONLY: settled fills only (IntentFulfilled), never a pending intent, never trades
//   ZERO-KNOWLEDGE: the chain shows token/amount/platform/rate, NEVER the Venmo side or
//     counterparty — "the chain never shows it, so neither does Casberi"
//   read off Base's public chain; brand indigo #4b47f6, swap-arrows glyph (from website)
// Drawn mark (the site's own swap glyph), no external logos.
const fs=require('fs'), path=require('path');
const AC='#4B47F6', VEN='#3D95CE', GRN='#3fb950', GOLD='#E0982A';
const BEATS=[
  {kick:'FILLS',    head:'Your Peer\ntrades.',      accent:AC},
  {kick:'NO ACCOUNT',head:'No account.\nNo key.',   accent:AC},
  {kick:'SETTLED',  head:'As they\nsettle.',         accent:AC},
  {kick:'PRIVATE',  head:'Onchain, yet\nprivate.',   accent:AC},
  {kick:'READ-ONLY',head:'It never\ntrades.',        accent:AC},
];
...
// title() outputs, honest degradation shown across the rows
const FILLS=[
  ['Bought 25 USDC','with Venmo','Venmo',VEN,'settled · just now'],
  ['Bought 0.05 ETH','with PayPal','PayPal','#0070E0','settled · 2h ago'],
  ['Bought 120 USDT','with Revolut','Revolut','#191c1f','settled · yesterday'],
];
// what the chain reveals vs what the ZK proof keeps private
const SHOWN=[[1,'The token','USDC'],[1,'The amount','25'],[1,'The payment app','Venmo'],[1,'The rate','1.00']];
const HIDDEN=[[0,'Your Venmo handle',''],[0,'Who you traded with',''],[0,'Your bank, your balance','']];
...
/* fills */
.frow{display:flex;align-items:center;gap:24px;padding:28px 0;border-top:2px solid rgba(255,255,255,.07);will-change:opacity,transform;}
.frow:first-of-type{border-top:none;}
.ftile{width:80px;height:80px;border-radius:20px;flex:none;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:800;color:#fff;}
.ft{font-size:36px;font-weight:650;} .ft b{font-weight:800;}
.fvia{font-size:24px;color:rgba(255,255,255,.45);margin-top:6px;} .fvia b{color:rgba(255,255,255,.7);font-weight:600;}
.fset{margin-left:auto;font-size:23px;color:${GRN};font-weight:600;text-align:right;}
/* no account — toggle over wallet */
...
.wav{width:76px;height:76px;border-radius:50%;flex:none;background:conic-gradient(from 210deg,#7b8cff,#2461ff,#7b8cff);}
...
.togl{font-size:33px;font-weight:600;}
...
.wnote b{color:#fff;}
/* settled */
.srow{display:flex;align-items:center;gap:22px;padding:26px 30px;border-radius:18px;margin-bottom:20px;font-size:31px;will-change:opacity,transform,filter;}
...
.srow .stag{margin-left:auto;font-size:24px;font-weight:700;}
...
.pcS{background:rgba(75,71,246,.1);box-shadow:inset 0 0 0 2px rgba(75,71,246,.4);}
.pcH{background:rgba(255,255,255,.03);box-shadow:inset 0 0 0 2px rgba(255,255,255,.1);}
.pch{font-size:22px;letter-spacing:.1em;margin-bottom:22px;font-weight:700;}
.pcS .pch{color:#9a97ff;} .pcH .pch{color:rgba(255,255,255,.4);}
.pr{display:flex;align-items:center;gap:14px;padding:16px 0;font-size:27px;font-weight:600;will-change:opacity;}
.pr i{width:34px;height:34px;border-radius:50%;flex:none;display:flex;align-items:center;justify-content:center;font-style:normal;font-size:19px;font-weight:800;}
...
.pcH .pr em{font-style:normal;}
.pnote{margin-top:30px;font-size:27px;color:rgba(255,255,255,.5);line-height:1.45;will-change:opacity;}
.pnote b{color:#fff;}
/* read-only */
.vrow{display:flex;align-items:center;gap:24px;padding:30px 0;border-top:2px solid rgba(255,255,255,.07);font-size:38px;font-weight:600;will-change:opacity,transform;}
.vrow:first-of-type{border-top:none;}
.vrow s{width:60px;height:60px;border-radius:50%;flex:none;background:rgba(255,255,255,.06);color:rgba(255,255,255,.3);display:flex;align-items:center;justify-content:center;font-size:32px;text-decoration:none;}
.vrow b{color:#fff;}
.vok{display:flex;align-items:center;gap:20px;margin-top:34px;font-size:32px;font-weight:650;color:${GRN};will-change:opacity,transform;}
.vok i{width:52px;height:52px;border-radius:50%;flex:none;background:${GRN};color:#04140a;display:flex;align-items:center;justify-content:center;font-style:normal;font-size:30px;}
...
<div class="stage">
  <div class="grain"></div>
  <div class="mast mono"><span>CASBERI</span><span>PEER</span></div>
  <div class="rule" id="rule"></div>
  <div class="wm" id="wm">01</div>

  <div class="comp" id="comp0">
    <div class="shd mono"><span>SETTLED FILLS</span></div>
    ${FILLS.map((f,i)=>`<div class="frow" data-i="${i}"><span class="ftile" style="background:${f[3]}"><svg viewBox="0 0 24 24" width="40" height="40"><path fill="#fff" d="M6.99 11 3 15l3.99 4v-3H14v-2H6.99v-3zM21 9l-3.99-4v3H10v2h7.01v3L21 9z"/></svg></span><div><div class="ft">Bought <b>${f[0].replace('Bought ','')}</b></div><div class="fvia"><b>${f[2]}</b> on Peer</div></div><span class="fset">${f[4]}</span></div>`).join('')}
  </div>
  ...
  <div class="comp" id="comp1">
    ...
    <div class="wcard">
      ...
      <div class="togrow"><span class="togl">Land my Peer trades</span><span class="tog" id="tog"><span class="knob" id="knob"></span></span></div>
    </div>
    <div class="wnote" id="wnote">Peer's identity <b>is your wallet</b> — like Strava rides Apple Health. One switch over a wallet you already watch. No sign-up, no key.</div>
  </div>

  <div class="comp" id="comp2">
    <div class="shd mono"><span>ON BASE'S PUBLIC CHAIN</span></div>
    <div class="srow pend" id="spend"><span class="si">…</span><span>Trade signaled</span><span class="stag mono">pending</span></div>
    <div class="srow done" id="sdone"><span class="si">✓</span><span>Bought <b>25 USDC</b> with Venmo</span><span class="stag mono">settled</span></div>
    <div class="snote" id="snote">A fill lands the moment it <b>settles onchain</b> — never a pending intent the app would have to guess at. Only what actually happened.</div>
  </div>

  <div class="comp" id="comp3">
    <div class="shd mono"><span>ZERO-KNOWLEDGE BY DESIGN</span></div>
    <div class="pcol">
      <div class="pc pcS" data-i="0"><div class="pch mono">THE CHAIN SHOWS</div>${SHOWN.map(s=>`<div class="pr"><i>✓</i>${s[1]}</div>`).join('')}</div>
      <div class="pc pcH" data-i="1"><div class="pch mono">STAYS PRIVATE</div>${HIDDEN.map(s=>`<div class="pr"><i>🔒</i>${s[1]}</div>`).join('')}</div>
    </div>
    <div class="pnote" id="pnote">Peer proves the fiat side without revealing it. The chain never shows your Venmo — <b>so neither does Casberi</b>.</div>
  </div>

  <div class="comp" id="comp4">
    <div class="shd mono"><span>CAPTURE ONLY</span></div>
    <div class="vrow" data-i="0"><s>✕</s><span>Never <b>starts</b> a trade</span></div>
    <div class="vrow" data-i="1"><s>✕</s><span>Never <b>signs</b> or settles</span></div>
    <div class="vok" id="vok"><i>✓</i> Only reads the fills, once they're done</div>
    <div class="wnote" style="margin-top:36px" id="vnote">Settled trades land in your feed and stop there. The same line the whole wallet holds — <b>read-only</b>.</div>
  </div>
...
function animateComp(i,p,t){
  if(i===0){
    document.querySelectorAll('#comp0 .frow').forEach((r,k)=>{const rp=clamp01((p-k*0.14)/0.36);r.style.opacity=rp;r.style.transform='translateX('+((1-easeOut(rp))*-34)+'px)';});
  } else if(i===1){
    const on=clamp01((p-0.34)/0.24);
    ...
    document.getElementById('wnote').style.opacity=clamp01((p-0.52)/0.3);
  } else if(i===2){
    ...
    const settle=clamp01((p-0.4)/0.26);
    ...
    document.querySelectorAll('#comp3 .pc').forEach((r,k)=>{const rp=clamp01((p-k*0.16)/0.4);r.style.opacity=rp;r.style.transform='translateY('+((1-back(rp))*28)+'px)';
      r.querySelectorAll('.pr').forEach((row,j)=>{row.style.opacity=clamp01((p-0.2-k*0.12-j*0.07)/0.24)*(r.classList.contains('pcH')?0.999:1);});});
    document.getElementById('pnote').style.opacity=clamp01((p-0.58)/0.3);
  } else if(i===4){
...
fs.writeFileSync(path.join(__dirname,'clip-peer-live.html'),html);
console.log('wrote clip-peer-live.html',(html.length/1024).toFixed(0)+'KB',TOTAL.toFixed(1)+'s');
