// Captures clip-farcaster.html in REAL TIME (it plays real <video> elements)
// via Playwright's built-in video recorder, then reports the output path.
const { chromium } = require('/opt/homebrew/lib/node_modules/playwright');
const fs = require('fs');
const path = require('path');

const OUT_DIR = path.join(__dirname, 'rec-farcaster-out');
...
(async () => {
  ...
  await page.goto('file://' + path.join(__dirname, 'clip-farcaster.html'));
