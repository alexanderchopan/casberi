...
//   node render.js clip-farcaster-motion.html --size=1080x1920
const fs = require('fs');
const path = require('path');

const CROPS = '/Users/alexanderchopan/Developer/casberi/scratchpad/fc-shots/crops';
...
const BEATS = [
  { imgs: ['store.png'],               head: 'Add Farcaster',        sub: 'Right in the app store.' },
  { imgs: ['vitalik.png'],             head: 'Follow anyone',        sub: 'By name — here, vitalik.eth.' },
  { imgs: ['cast1.png', 'cast2.png'],  head: 'Casts in your feed',   sub: 'Plus their likes and mentions.' },
  { imgs: ['replies.png'],             head: 'See the whole thread', sub: 'Every reply, faces and all.' },
  { imgs: ['home.png'],                head: 'All on your Home',     sub: 'Where your day comes together.' },
];
...
.stage{position:absolute;inset:0;overflow:hidden;background:
  radial-gradient(1200px 900px at 50% 8%, #171226 0%, #0a0a12 55%, #08080e 100%);}
.glow{position:absolute;inset:-25%;will-change:transform;
  background:radial-gradient(760px 620px at 50% 46%, rgba(139,92,246,.28), transparent 60%),
             radial-gradient(680px 560px at 74% 84%, rgba(76,29,149,.18), transparent 62%);}
...
.spot{position:absolute;left:50%;top:1010px;width:1040px;height:1040px;transform:translate(-50%,-50%);
  background:radial-gradient(closest-side, rgba(139,92,246,.16), transparent 72%);will-change:opacity;opacity:0;}
...
fs.writeFileSync(path.join(__dirname, 'clip-farcaster-motion.html'), html);
console.log('wrote clip-farcaster-motion.html', (html.length/1024/1024).toFixed(1)+'MB', TOTAL.toFixed(1)+'s');
