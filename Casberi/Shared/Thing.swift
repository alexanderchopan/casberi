...
enum Corpus {
    ...
    /// seat has no catalog entry, no chip, no room and no deep-link door.
    /// Declared HERE rather than derived from `BridgeCatalog` (which the
    /// share extension cannot see) so `earnsRoom` stays the one rule every
    /// surface reads; a source in this set and in the catalog at once would
