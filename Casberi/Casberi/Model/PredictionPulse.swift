...
final class PredictionPulse {
    ...
    // `seedDemo`/`reseedDemoIfNeeded`/`teardownDemo` lived here from
    // 2026-08-12 until 2026-09-06 (prd §638): the Kalshi and Polymarket seats
    // left the catalog, the demo seeds no market rows, and a cache seeded for
    // rows that never land is the fake status it was written to prevent.
