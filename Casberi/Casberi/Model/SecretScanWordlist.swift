...
/// The BIP39 English wordlist — the 2,048 words every standard crypto
/// recovery phrase is drawn from, and the only reason `SecretScan` can tell
/// a recovery phrase from an ordinary list of words with any precision.
///
/// Fetched verbatim 2026-08-02 from the BIPs repository
/// (bitcoin/bips, bip-0039/english.txt), SHA-256
/// 2f5eed53a4727b4bf8880d8f3f199efc90e58503646d9ff8eff3a2ed3b24dbda
/// — recorded so a future edit can be checked against its source rather
/// than trusted. Do NOT hand-edit: a wrong word is a silent false negative
/// on the one detector whose miss is expensive.
///
/// Lives in its own file so `SecretScan` stays readable. The set is built
/// once, lazily, on the first scan that needs it.
enum SecretScanWordlist {
    static let english: Set<String> = Set(
        raw.split(whereSeparator: { $0.isWhitespace }).map(String.init))

    /// 2,048 words, 12 per line, in the list's own order.
