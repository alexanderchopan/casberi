...
final class MCPServer {
    ...
    private static let toolDeclarations: [[String: Any]] = [
        ["name": "search_things",
         "description": "Keyword search over the things this person has saved in Casberi — links, notes, screenshots, chats, events, transactions and more. A term matches anywhere in a thing's title, tags or text; title matches rank highest and something saved in the last week ranks above an older equal match. There is no semantic matching, so a synonym finds nothing. Only the newest 500 things are searched. Each result is one line — title, kind, source app — with no body text, and any credential-shaped text is masked. Says so in a sentence when nothing matches.",
         "inputSchema": ["type": "object",
                         "properties": ["query": ["type": "string",
                                                  "description": "Words to search for. Case-insensitive; the common words 'about', 'my', 'the', 'a', 'in', 'from', 'for' and 'of' are ignored."],
                                        "limit": ["type": "integer",
                                                  "description": "How many to return, 1 to 25. Defaults to 10."]],
                         "required": ["query"]]],
        ["name": "week_synthesis",
         "description": "One sentence about the last seven days of saves: how many things landed, which tag led and with how many, and how many different apps they came from. Counted rather than written by a model, so it is the same for the same corpus. Carries no titles or body text. Takes no arguments.",
         "inputSchema": ["type": "object", "properties": [String: Any]()]],
        ["name": "save_thing",
         "description": "Propose something to save into Casberi. This does NOT save it: it lands in the app as an item awaiting the person's approval, and only their tap commits it. Returns a confirmation that the proposal is waiting, never a saved item — so do not tell the person it has been saved.",
         "inputSchema": ["type": "object",
                         "properties": ["text": ["type": "string",
                                                 "description": "The text to save. Its first line, up to 60 characters, becomes the item's title."],
                                        "tags": ["type": "array", "items": ["type": "string"],
                                                 "description": "Optional tags, without the leading '#'. They ride in the saved text and are applied if the person approves."]],
                         "required": ["text"]]],
    ]
