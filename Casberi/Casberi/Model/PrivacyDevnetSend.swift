...
enum PrivacyDevnetSend {
    ...
    /// Shield `weiHex` into the pool as a new note (prd §593e).
    ///
    /// The deposit's amount is public; what shielding hides is the LINK between
    /// this deposit and where the money later leaves. A fresh note secret
    /// `rho` (generated by the caller, exactly as `freshNonceKey` is — a
    /// predictable secret is worse than an honest one, so a phone that cannot
    /// make one drops the act rather than shielding weak) plus this book's
    /// spend key give the note's `inner`; the pool recomputes the commitment
    /// itself from `inner` and `msg.value`, so the wire carries only `inner`.
    ///
    /// **The note is recorded ONLY after the node accepts the broadcast.** A
    /// note stored for a transaction the node refused would inflate the
    /// shielded balance forever with money that never entered the pool — which
    /// is exactly why View re-checks every stored note against the pool's real
    /// leaves before counting it.
    ///
    /// Every number here was verified in `privacy-poseidon-selftest.sh` against
    /// a real shield this project landed on chain 8141.
    static func shield(weiHex: String, rho: Data) async throws -> String {
        guard let address = PrivacyDevnetKey.address() else { throw Failure.noKey }
        let sk = PrivacyDevnetNoteBook.spendKey()
        let rhoFp = PrivacyDevnetNote.fp(bytesBE: [UInt8](rho))
        let inner = PrivacyDevnetNote.inner(spendKey: sk, rho: rhoFp)
        let value = PrivacyDevnetNote.fp(weiHex: weiHex)
        let commitment = PrivacyDevnetNote.commitment(inner: inner, value: value)
        let calldata = Data(PrivacyDevnetNote.shieldCalldata(inner: inner))

        guard let nonce = await currentNonce(for: address) else { throw Failure.chainUnreachable }
        guard let gasPrice = await suggestedGasPrice() else { throw Failure.chainUnreachable }
        let fields = shieldFields(weiHex: weiHex, calldata: calldata, nonce: nonce, gasPrice: gasPrice)
        let hash = try await send(fields)

        PrivacyDevnetNoteBook.record(.init(
            rhoHex: hex(rhoFp.bytesBE()),
            valueWeiHex: weiHex,
            commitmentHex: hex(commitment.bytesBE()),
            createdAt: Date()))
        return hash
    }

    /// The shield transaction: the same self-verifying two-frame shape as an
    /// ordinary transfer, with the SENDER frame targeting the POOL and carrying
    /// `shield(inner)` plus the deposit as `msg.value`.
    ///
    /// **The budgets are the ones measured on this chain**: a shield runs
    /// Poseidon and a tree insert, so its sender frame needs far more than a
    /// plain transfer's, and — the Frames-devnet lesson (§548) — a real
    /// state budget, because a shield GROWS state and execution gas cannot pay
    /// for that. `0x16e360`/`0xc3500` (1,500,000 / 800,000) is what the live
    /// shield used.
    static func shieldFields(weiHex: String, calldata: Data,
                             nonce: UInt64, gasPrice: UInt64) -> PrivacyDevnetTransaction.Fields {
        let sender = PrivacyDevnetKey.address() ?? ""
        let senderBytes = PrivacyDevnetRPC.hexData(sender)
        let verify = PrivacyDevnetTransaction.Frame(
            mode: 1, flags: 3, target: senderBytes,
            gasLimit: 0x13880, stateLimit: 0, value: Data(), data: Data())
        let deposit = PrivacyDevnetTransaction.Frame(
            mode: 2, flags: 0, target: PrivacyDevnetRPC.hexData(PrivacyDevnetPool.address),
            gasLimit: 0x16e360, stateLimit: 0xc3500,
            value: PrivacyDevnetRPC.hexData(weiHex), data: calldata)
        return PrivacyDevnetTransaction.Fields(
            chainID: PrivacyDevnetChain.chainID,
            nonceKeys: [Data([0])],
            nonce: nonce,
            sender: senderBytes,
            frames: [verify, deposit],
            signatures: [],
            maxPriorityFeePerGas: gasPrice,
            maxFeePerGas: gasPrice,
            maxFeePerBlobGas: 0,
