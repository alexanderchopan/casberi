...
///
/// # The order: BIGGEST FIRST, and it is measured
///
...
///
/// So the whole optimisation is the sort, and the DP was deliberately NOT
/// shipped: it never once won, and it would have cost a scrambled order to run.
/// It lives in the harness instead, where it proves the sort still ties it.
/// **Do not replace this with a cleverer packer without re-running that
/// comparison — there is no room above it to win.**
///
/// Ties break on the order given (catalog order), so a packing is deterministic
/// for a given corpus rather than depending on dictionary iteration.
enum SourceRowPacking {
    ...
    ///
    /// `catalog` arrives in catalog order and is used for the size tiebreak;
    /// the returned rows are in packing order. Empty categories are dropped
    /// rather than drawn as an empty card.
    static func pack(_ catalog: [Block]) -> [[Block]] {
        let blocks = catalog.filter { !$0.members.isEmpty }

        // Biggest first — the entire optimisation. See the type doc.
