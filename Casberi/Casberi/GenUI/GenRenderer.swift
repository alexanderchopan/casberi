    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text(el.str(0))
                ...
                .padding(.init(top: DS.Space.s4, leading: DS.Space.s4,
                               bottom: DS.Space.s1, trailing: DS.Space.s4))
            // The single row inline — the resolved ref draws, an unresolved one
            // (still streaming) simply drops, the mount law without a skeleton
            // nest.
            ForEach(el.refs(1), id: \.self) { ref in
                if let child = els[ref] { rowLine(child) }
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(.bottom, DS.Space.s2)
